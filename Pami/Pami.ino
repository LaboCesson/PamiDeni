...
void timedActionsManager(){
  ...
  if (millis() > timedAction) {
      if (timedActionSelector == 1) {
        ...
        Serial.println("################## Executed timed Action.");
      }
  }
}
  ...
  if (zbeub && angle < slopeMin) {
    ...
    switch (actionSelector) {
      ...
      case 0x02:
        ...
        Serial.println("conditionnalCorrection has set targetAngleOffset to 90.");
        zbeub = false;
        break;
...
void onEdge() {
  if (awaitingDistance) {
    ...
    if ((short)distance >= (short)(50)) { // Hauteur réelle: 55mm (5.5cm). Changer si besoin pour correspondre aux millimètres!!
      pami.moteur.moteurGauche(0);
      pami.moteur.moteurDroit(0);
      coOverride = true;
