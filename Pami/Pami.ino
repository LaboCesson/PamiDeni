...
void correctDependingOnAngle() {
  ...
  if (!coOverride){
    short md = max(min(direction * (baseSpeed - (angle + targetAngleOffset)), 100), -100);
    short mg = max(min(direction * (baseSpeed + (angle + targetAngleOffset)), 100), -100);
    ...
    Serial.print("md: ");
    Serial.print((short)md);
    Serial.print(" mg: ");
    Serial.println((short)mg);
...
void onEdge() {
  if (awaitingDistance) {
    ...
    // HAUTEUR DE TETECTION CHANGEE POUR EVITER AU MAXIMUM LES FAUX POSITIFS!!

    if ((short)distance >= (short)(70) && (short)distance <= (short)100) { // Hauteur réelle: 55mm (5.5cm). Changer si besoin pour correspondre aux millimètres!!
